...
# BCM Data Exports is managed through the us-east-1 service endpoint while
# the report destination remains the existing eu-west-2 S3 bucket.
resource "aws_bcmdataexports_export" "cur2" {
  ...
  export {
    ...
    data_query {
      ...
      # AWS returns these values on the existing export. Keeping all observed
      # keys explicit avoids provider drift on computed table configuration.
